...
const upToQiniu = (filePath, key) => {
  ...
  // 添加七牛图片处理参数，此处以压缩为例
  const imageProcessingOptions = "imageView2/2/w/800/q/75"; // 设置压缩后的宽度和质量

  const formUploader = new qiniu.form_up.FormUploader(config);
  const putExtra = new qiniu.form_up.PutExtra();

  return new Promise((resolve, reject) => {
    formUploader.putStream(uploadToken, key, filePath, putExtra, function (respErr, respBody, respInfo) {
      if (respErr) {
        reject(respErr);
      } else {
        resolve(respBody);
